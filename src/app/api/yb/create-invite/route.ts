...
export const POST = createZodRoute()
  ...
  .handler(async (r, ctx) => {
    ...
    if (!code) {
      return new NextResponse(JSON.stringify({ status: 400, err: "Code Not Specified" }), { status: 400 })
    }

    const insRes = await db
      .insert(invite)
      .values({ code: code, createdBy: apiQ[0].owner })
      .onConflictDoNothing({ target: invite.code })
      .returning()

    if (insRes.length < 1) {
      if (rnd) {
        return new NextResponse(JSON.stringify({ status: 500, err: "Random Value Collision. Try again." }), {
          status: 500,
        })
      }
      return new NextResponse(JSON.stringify({ status: 400, err: `Code ${code} already existed!` }), {
        status: 400,
      })
    }
