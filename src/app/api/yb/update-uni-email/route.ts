...
export const POST = createZodRoute()
  ...
  .handler(async (r, ctx) => {
    ...
    if (stdq.length < 1)
      return new NextResponse(JSON.stringify({ status: 400, err: "Bad data" }), { status: 400 })

    const std = stdq[0]

    if (std.emailuni !== email)
      await db.update(thirtyeight).set({ emailuni: email }).where(eq(thirtyeight.stdid, std.stdid))
