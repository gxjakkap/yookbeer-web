...
import { addAPIKey, deleteAPIKey, editAPIKey } from "@/app/(authorized)/admin/actions"
			...
			<DialogContent className="sm:max-w-md">
				...
				<form onSubmit={handleSubmit}>
					<div className="flex flex-col gap-y-4">
						...
						<div>
							<Label className="text-right capitalize" htmlFor="name">
								Name
							</Label>
							<Input
								id="name"
				...
				<form action={() => onUpdate(formData)}>
					<div className="grid gap-4 py-4">
						<div className="grid grid-cols-4 items-center gap-4">
							<Label className="text-right capitalize">Expiration Date</Label>
							<Popover>
								<PopoverTrigger asChild>
									<Button
										variant={"outline"}
...
const DeleteDialog = ({ isOpen, onClose, onConfirm, apiKey, isPending }: DeleteDialogProps) => {
	return (
		<Dialog open={isOpen} onOpenChange={onClose}>
			<DialogContent className="sm:max-w-md">
				<DialogHeader>
					...
					<DialogDescription>
						Are you sure you want to delete API key {apiKey}? This action cannot be undone.
					</DialogDescription>
				</DialogHeader>
				<DialogFooter>
					<Button type="button" variant="outline" onClick={onClose} disabled={isPending}>
...
const createColumns = (): ColumnDef<YookbeerAPIKeyColumn>[] => [
	...
	},
	{
		...
		cell: ({ row }) => {
			...
			return <div>{dateValue ? format(dateValue as Date, "PPP") : "No expiration"}</div>
		},
	},
	{
		accessorKey: "action",
...
const filterKeys: Array<keyof YookbeerAPIKeyColumn> = ["id", "key"]
