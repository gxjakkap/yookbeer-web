...
const AddDialog = ({ isOpen, onClose, onAdd, isPending }: AddDialogProps) => {
  const addFormSchema = z.object({
    code: z
      .string()
      .length(0, { message: "Custom invite code must be at least 4 characters long" })
      .or(z.string().min(4, { message: "Custom invite code must be at least 4 characters long" }))
      .optional(),
  })
...
const DeleteDialog = ({ isOpen, onClose, onConfirm, code, isPending }: DeleteDialogProps) => {
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>
            Are you sure you want to delete invite code <span className="font-bold">{code}</span>? This action
            cannot be undone.
          </DialogDescription>
        </DialogHeader>
        <DialogFooter>
...
const createColumns = (): ColumnDef<YookbeerInviteColumn>[] => [
  ...
  },
  {
    ...
    cell: ({ row }) => {
      ...
      return <div>{dateValue ? format(dateValue as Date, "PPP") : ""}</div>
    },
  },
  {
  ...
  },
  {
    accessorKey: "usedBy",
    header: "Used by",
    cell: ({ row }) => (row.getValue("usedBy") ? <div>{row.getValue("usedBy")}</div> : <></>),
  },
  {
...
const filterKeys: Array<keyof YookbeerInviteColumn> = ["id", "code"]
