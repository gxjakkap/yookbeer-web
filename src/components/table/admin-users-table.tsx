...
export interface YookbeerUserColumn {
	...
	emailVerified:
		| SQL<unknown>
		| Date
		| PgColumn<ColumnBaseConfig<ColumnDataType, string>, never, never>
		| null
		| undefined
	image: string | null
	role: string | null
				...
				<form action={() => onUpdate(formData)}>
					<div className="grid gap-4 py-4">
						<div className="grid grid-cols-4 items-center gap-4">
							<Label className="text-right capitalize">Role</Label>
							<DropdownMenu>
								<DropdownMenuTrigger asChild>
									<Button variant="outline" className="w-32">
										{formData.role?.replace(/([A-Z])/g, " $1").trim()}
									</Button>
								</DropdownMenuTrigger>
								<DropdownMenuContent className="w-56">
									...
									>
										...
										<DropdownMenuRadioItem value="unauthorized">
											No Access
										</DropdownMenuRadioItem>
									</DropdownMenuRadioGroup>
								</DropdownMenuContent>
...
const DeleteDialog = ({ isOpen, onClose, onConfirm, userName, isPending }: DeleteDialogProps) => {
	return (
		<Dialog open={isOpen} onOpenChange={onClose}>
			<DialogContent className="sm:max-w-md">
				<DialogHeader>
					...
					<DialogDescription>
						Are you sure you want to delete the record for {userName}? This action cannot be
						undone.
					</DialogDescription>
				</DialogHeader>
				<DialogFooter>
...
const filterKeys: Array<keyof YookbeerUserColumn> = ["id", "name", "email"]

export function AdminUserTable({ data }: YookbeerUserTableProps) {
	const [sorting, setSorting] = React.useState<SortingState>([])
	const [globalFilter, setGlobalFilter] = React.useState<string>("")
	const [columnVisibility, setColumnVisibility] = React.useState<VisibilityState>({ emailper: false })
