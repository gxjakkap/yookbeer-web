...
const formSchema = z.object({
    ...
    including: z.array(
        z.enum(INCLUDABLE.map((i) => i.id) as [typeof INCLUDABLE[number]["id"], ...string[]])
    ).min(1, "Select at least one field"),
    format: z.enum(["csv", "json"]),
    includedCourse: z.array(
        z.enum(AVAILABLE_COURSES.map((c) => String(c.id)) as [string, ...string[]])
    )
...
export default function TakeoutForm(){
    ...
    return (
        <Card className="mx-10 my-5">
            <CardContent>
                <Form {...form}>
                    <form onSubmit={form.handleSubmit(onSubmit)} className="flex flex-col my-5 gap-4">
                        <div className="flex flex-col lg:flex-row lg:justify-center gap-x-60 gap-y-4">
